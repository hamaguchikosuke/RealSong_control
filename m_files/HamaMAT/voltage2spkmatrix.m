...
% vtmp(p_spk_ind)
    ...
    if length(p_spk_ind)>1  % and then, if there is more than one spike in this row, we need to think about refractoriness.
                            % Why not simply set the initial refractoriness
                            % Inf? It is because whether a spike occurs within a
                            % refractoriness or not depends on whether the
                            % most recent previous spikes are removed or not.
                            % Because of that, it is simpler to work from
                            % earlier to later spikes at a time. 
