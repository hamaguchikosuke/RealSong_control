function [h,p,ci,stats] = my_ttest2(x,y,alpha,tail,vartype,dim)
%MY_TTEST2 Two-sample T-test with pooled or unpooled variance estimate.
%   H = MY_TTEST2(X,Y) performs a T-test of the hypothesis that two
...
%
%   X and Y can also be matrices or N-D arrays.  For matrices, MY_TTEST2
...
%   arrays, MY_TTEST2 works along the first non-singleton dimension.  X and Y
%   must have the same size along all the remaining dimensions.
%
%   MY_TTEST2 DOES NOT treats NaNs as missing values, and ignores them.
%
%   H = MY_TTEST2(X,Y,ALPHA) performs the test at the significance level
%   (100*ALPHA)%.  ALPHA must be a scalar.
%
%   H = MY_TTEST2(X,Y,ALPHA,TAIL) performs the test against the alternative
...
%
%   H = MY_TTEST2(X,Y,ALPHA,TAIL,VARTYPE) allows you to specify the type of
%   test.  When VARTYPE is 'equal', MY_TTEST2 performs the default test
%   assuming equal variances.  When VARTYPE is 'unequal', MY_TTEST2 performs
...
%   problem. MY_TTEST2 uses Satterthwaite's approximation for the effective
%   degrees of freedom.  VARTYPE must be a single string.
%
%   [H,P] = MY_TTEST2(...) returns the p-value, i.e., the probability of
%   observing the given result, or one more extreme, by chance if the null
%   hypothesis is true.  Small values of P cast doubt on the validity of
%   the null hypothesis.
%
%   [H,P,CI] = MY_TTEST2(...) returns a 100*(1-ALPHA)% confidence interval for
%   the true difference of population means.
%
%   [H,P,CI,STATS] = MY_TTEST2(...) returns a structure with the following fields:
...
%
%   [...] = MY_TTEST2(X,Y,ALPHA,TAIL,VARTYPE,DIM) works along dimension DIM of
...
s2x = var(x,[],dim);
s2y = var(y,[],dim);
difference = mean(x,dim) - mean(y,dim);
