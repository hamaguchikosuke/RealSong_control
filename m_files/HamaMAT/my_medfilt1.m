function y = my_medfilt1(x,n,blksz,DIM)
% MY_MEDFILT1  One dimensional median filter.
%   medfilt1 does zero-padding before and after the sequence.
%   It causes sharp-transition from zero to its mean at the beginning and
%   the end. To avoid this,
%   my_medfilt1 does mean-of-x padding before and after the sequence.
% 
...
%   $Revision: 1.8.4.3 $  $Date: 2007/12/14 15:05:24 $
...
meanx = nanmean(x);
X = [repmat(meanx,[m,1]); x; repmat(meanx,[m,1])];
y = zeros(nx,1);
