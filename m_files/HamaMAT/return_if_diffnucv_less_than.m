function [nu,nuext_grid,cv]=...
    return_if_diffnu_less_than(dnucv,rate_range,cnuext,known_nu,p,varargin)
% [nu,nuext_grid,cv]=return_if_diffnu_less_than(dnucv,rate_range,cnuext,known_nu,p)
% dnucv = [dnu,dcv] is the target difference of the rate and cv.
% if dnucv is scalar, rate is the only target.
% rate_range = [min_rate,max_rate]; %output rate should cover this range.
% cnuext = [min_nuext,max_nuext] is the initial input rate.
...
% p.Cnuext= [.5:.5:2];
% 
% [nu,nuext_grid,cv]=return_if_diffnucv_less_than(dnu,rate_range,cnuext,known_nu,p)
% by K.H. 20072120
% -----------------------------------------------------------------------
...
if ((diff(known_nu)>dnu) ||...
    ...
    [nutmp1,nuext_grid1,cvtmp1]=feval(mfilename,dnu,[cnuext(1),mean(cnuext)],[known_nu(1),NaN],p,[known_cv(1),NaN]);
    if ((abs(known_nu(2)-nutmp1(end))>dnu) || ...
        ...
        [nutmp2,nuext_grid2,cvtmp2]=feval(mfilename,dnu,[mean(cnuext),cnuext(2)],[nutmp1(end),known_nu(2)],p,[cvtmp1(end),known_cv(2)]);
    else
