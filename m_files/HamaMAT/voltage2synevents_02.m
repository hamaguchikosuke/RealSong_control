function [synmatrix,varargout] = voltage2synevents_02( ...
...
%
% %Usage: 
%  synmatrix = voltage2synevents_02( ...
...
%  dt is in ms.  
%  synmatrix = voltage2synevents_02(v,threshold,ref,opt,dt)    
% 
% %Usage(3)
% [synmatrix, extremal_matrix, Dextremal_matrix]=...
%  voltage2synevents_02(voltage,threshold,refractoriness,F,opt) 
% %Output Arguments:
...
if ~( isnumeric(voltage) && length(voltage) > 1 )
  error( 'voltage2synevents_02: 1st argument is not a numeric vector of length >= 2' )
end

if ~( isscalar(threshold) && isnumeric(threshold) )
  error( 'voltage2synevents_02: 2nd parameter threshold is not a numerical scalar.' )
end

if ~( isscalar(t_ref) && isnumeric(t_ref) )
  error( 'voltage2synevents_02: 3rd parameter t_ref is not a numerical scalar.' )
end
...
elseif strmatch('i',loewr(opt))
    ...
    error('voltage2synevents_02: 4th parameter opt should be {''e'',''I''}' );
end
...
%   error('voltage2synevents_02: 2nd parameter threshold is outside range of voltage')
% end
...
fprintf('synaptic event detection:\nthreshold=%d*std[dv/dt], dt=%3.1g[ms], ref=%3.1g[ms], F=%d, detection=%s.\n',...
   ...
   %% First, determine the synaptic event timings
    ...
    for jj=1:length(p_syn_ind_start)
        ...
        if (length_above_threshold > min_exceeding_period) && (ISI>ref_bin)
            synmatrix(ii,p_syn_ind_start(jj))=1;
        end
    end
