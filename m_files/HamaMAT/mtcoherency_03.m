function [mtcoh_mat,fo,to,stat,varargout]=mtcoherency_03(x,y,D,nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers)
% [coh,fo,to] = mtcoherency_02(X,Y,D,nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers)
% [coh,fo,to,stat,Cxy,Axx,Ayy] = ...
%         mtcoherency_02(X,Y,D,nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers)
...
% % coh = mtcoherency_02(x',y',D,nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers);
...
% 
%  clf;plot(t,[x(1,:);y(1,:)]);
% [mtcoh_mat,fo,to,stat,cxy]=mtcoherency_03(x,y,D,nFFT,Fs,WinLength,nOverlap,NW,Detrend,nTapers);
% 
...
if (nargout>=5),    mtcxy_mat = zeros(2*D+1,nFFTChunks,nTrials); end
if (nargout>=6),    mtcxx_mat = zeros(2*D+1,nFFTChunks,nTrials); end
if (nargout>=7),    mtcyy_mat = zeros(2*D+1,nFFTChunks,nTrials); end
if (nargout>=8),    mtshiftcoh_mat = zeros(2*D+1,nFFTChunks,nTrials); end
...
for ii=1:nTrials
    ...
    % Calculate Coherency for each trial.
    ...
    if (nargout>=5)
        tmpF = [yo(:,:,2,1);conj(flipud(yo(2:end,:,1,2)))];
        mtc = real(ifft(tmpF,[],1));
        mtc = [mtc(end-D+1:end,:);mtc(1:D+1,:)];
        mtcxy_mat(:,:,ii) =mtc;
    end

    % Calculate auto-corariance
    if (nargout>=6)
        tmpF = [yo(:,:,1,1);flipud(yo(2:end,:,1,1))];
        mtc = real(ifft(tmpF,[],1));
        mtc = [mtc(end-D+1:end,:);mtc(1:D+1,:)];
        mtcxx_mat(:,:,ii) =mtc;
    end
    ...
    if (nargout>=7)
        tmpF = [yo(:,:,2,2);flipud(yo(2:end,:,2,2))];
        mtc = real(ifft(tmpF,[],1));
        mtc = [mtc(end-D+1:end,:);mtc(1:D+1,:)];
        mtcyy_mat(:,:,ii) =mtc;
    end
...
if (nargout>=5),    varargout{1}=mtcxy_mat; end

if (nargout>=6),    varargout{2}=mtcxx_mat; end
 
if (nargout>=7),    varargout{3}=mtcyy_mat; end

% ------------- Calculate Shuffled Corrector -----------%
if (nargout>=8),    varargout{4}=mtshiftcoh_mat; end
