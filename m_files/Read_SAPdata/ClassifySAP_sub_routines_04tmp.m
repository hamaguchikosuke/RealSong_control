...
%         ClassifySAP_sub_routines_04('init_feature_vector');
        ...
        sap.gph.ui_shown_cluster_as_noise=uicontrol('Style','pushbutton','String', 'Register as Noise',...
            'FontSize',8,...
            'Unit','Normalized','Position', [.85 .06 .1 .05],'Callback', 'ClassifySAP_sub_routines_04(''register_shown_cluster_as_noise'');');
        
        
        sap.gph.ui_register_shown_cluster_to_Manual_cluster=uicontrol('Style','pushbutton','String', 'Register',...
            'Unit','Normalized','Position', [.85 0 .1 .05],'Callback', 'ClassifySAP_sub_routines_04(''register_shown_cluster_to_Manual_cluster'');');
...
%         set(h,'FaceAlpha',0);
        ...
        save(fullfile(sap.pm.sound_dir,[sap.SAPtablename,'_transMatrix.mat']),...
...
% close(curs);close(conn);
        ...
        subplot(2,1,1);
        hist(sap.N_motif_counts,[0:1:max(sap.N_motif_counts)]);
        axis([0 max(sap.N_motif_counts) ylim]);
...
%                fprintf('look for %s in %s\n',search_pattern,str2);
    ...
    case 'print_sonogram'
        ...
        t=linspace(0,length(sap.pm.song)/sap.pm.Fsong,size(m_spec_deriv,1));
        f=sap.pm.Fsong*(1:size(m_spec_deriv,2))/1024;
        
       
        
        width = max(t)*4;         % Initialize a variable for width.
        height = 2;          % Initialize a variable for height.
        dx=.5;
        dy=1;
        papersize=[width+2*dx,height+2*dy];
        setPaperSize(papersize,12);clf;
%         set(gcf, 'PaperUnits', 'centimeters');
%         set(gcf, 'PaperSize', papersize);
    h=axes('Position',[dx/(width+2*dx),dy/(height+2*dy),width/(width+2*dx),height/(height+2*dy)]);
    imagesc(t,f,m_spec_deriv');
         colormap gray;
         cutoff=0.004;
         %                    text(min(xlim)+diff(xlim)*.05,max(ylim)-diff(ylim)*.1,sprintf('trial=%d',val));
