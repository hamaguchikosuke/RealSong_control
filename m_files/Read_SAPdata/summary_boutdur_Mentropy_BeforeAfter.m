...
% 
% load_table_names=[];
% load_table_names{1}='or831m_saline_0602_day2_x.MYD';
% load_table_names{2}='or831m_BMI10mM_0603_day3_x.MYD';
...
bout_dur_dirnames{1,4}='C:\home\LabVIEW\data\pur820m\pur820m_20100512_Undir_5mMMuscimol_day7_feature';
birdname{1}='pur820m';
...
bout_dur_dirnames{3,4}='C:\home\LabVIEW\data\sil827m\sil827m_20100519_muscimol_day5_feature';
birdname{3}='sil827m';


bout_dur_dirnames{4,1}='C:\home\LabVIEW\data\or831m\or831m_saline_0602_day2_feature';
bout_dur_dirnames{4,2}='C:\home\LabVIEW\data\or831m\or831m_BMI10mM_0603_day3_feature';
bout_dur_dirnames{4,3}='C:\home\LabVIEW\data\or831m\or831m_saline_0604_day4';

% % bout_dur_dirnames{3}='pur820m_20100508_undir_saline_day3_x.MYD';
    data_bouttmp=struct;
for jj=4:size(bout_dur_dirnames,1)
    h=[];h2=[];
    dur_t=[0:.1:8];
    motif_edge=[0:1:60];
    data_bout=struct;
    clf;
    axes_motif_N=subplot(2,1,1);
    axes_bout_dur=subplot(2,1,2);
    for ii=1:size(bout_dur_dirnames,2)
        ...
        load(loadname,'bout_duration','N_motif_counts');
        axes(axes_bout_dur);
        ...
        % pause
        axes(axes_motif_N);
        N=histc(N_motif_counts,motif_edge); N=N/sum(N);
        Ncdf=cumsum(N);
        
        h2(ii)=plot(motif_edge,Ncdf,ColOrder{ii}); hold on;
        data_bouttmp(jj,ii).Nmotif=N_motif_counts;
        data_bout(ii).Nmotif=N_motif_counts;
    end
    axes(axes_bout_dur);
    axis([0 7 0 1]);
    ...
    axis([0 60 0 1]);
    [is_significant,p_ks]=kstest2(data_bouttmp(jj,1).bout_dur,data_bouttmp(jj,2).bout_dur);
    xlabel('Number of motif in one bout');ylabel('Cumulative distribution of motif number')
    legend(h2,'pre','BMI','recovery','Muscimol')
    % text(max(xlim)-0.5*diff(xlim),min(ylim)+0.1*diff(ylim),...
    %     sprintf('pre VS BMI, KS-test (P=%g)',p_ks));
    title(sprintf('pre VS BMI, KS-test (p=%g)',p_ks));
