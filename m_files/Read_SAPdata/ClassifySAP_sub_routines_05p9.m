function varargout=ClassifySAP_sub_routines_05p9(opt,varargin)
...
switch lower(opt)
    case 'init_variables_for_figure'
        ...
        sap.pm.MaxNumSyllable=15;
        sap.pm.TotalNumCluster  = sap.pm.MaxNumSyllable+2;
        sap.pm.NoiseID          = sap.pm.MaxNumSyllable+1; % ID starts from 0.
        sap.pm.NoiseIndex       = sap.pm.MaxNumSyllable+2; % index starts from 1. 
        ...
        sap.pm.CS    = [...
            ...
            0.7500    0.2500   0.7500];             % identified noise
        
        sap.pm.CS =  generate_newColorOrder(sap.pm.CS);
        sap.pm.CS = [sap.pm.CS(1:(sap.pm.MaxNumSyllable+1),:); 0 0 0];
        
        sap.pm.max_load_N=50000;
    ...
    case 'set_teacher_data'
        ...
        switch d
            ...
            case 'from_another_database'
                ...
                conn_txt=sprintf('select * from %s where cluster>0 ',sap.Teachertablename);
                ...
                sap.data=cat(2,sap.data,tmpdata);
                % to delete previous teacher data.
...
%         myfigure('ClassidySAP')
        ...
        for ii=1:length(sap.pm.cluster_types)
            % evaltxt = sprintf('sap.gph.ui_cluster_type(%d) = uicontrol(''Style'',''Radio'',''Value'',0,''String'',sap.pm.cluster_types{%d},''unit'',''normalized'',''pos'',[%g,%g,%g,dy],''parent'',sap.gph.ui_cluster_type_select,''HandleVisibility'',''off'',''CallBack'',''ClassifySAP_sub_routines_05p9(''''%s'''')'');',...
        ...
        set(sap.gph.ui_cluster_type_select,'SelectionChangeFcn','ClassifySAP_sub_routines_05p9(''select_cluster_data_type'')');
        ...
        sap.gph.ui_teacher_cluster_ONOFF = uicontrol('Style','Radio',...
            ...
            'CallBack','ClassifySAP_sub_routines_05p9(''set_teacher_OnOff'');');
        sap.gph.ui_Stimlated_cluster_ONOFF = uicontrol('Style','Radio',...
            'Value',1,'String','stimulated','unit','normalized','pos',[.12 .90 .1 .03],...
            'CallBack','ClassifySAP_sub_routines_05p9(''set_Stimlated_cluster_ONOFF'');');
        ...
        sap.gph.ui_teacher_loadbutton = uicontrol('Style','pushbutton',...
            ...
            'CallBack','ClassifySAP_sub_routines_05p9(''load_teacher_database'');');
        ...
        dx = 1; dy=1/(sap.pm.MaxNumSyllable+2);
        for ii=[0:sap.pm.MaxNumSyllable]
            evaltxt = sprintf('sap.gph.ui_cluster%d = uicontrol(''Style'',''Radio'',''Value'',1,''String'',[''cluster'' num2str(%d) '':(--)''],''unit'',''normalized'',''pos'',[%g,%g,%g,dy],''parent'',sap.gph.ui_clusterselect,''HandleVisibility'',''off'',''CallBack'',''ClassifySAP_sub_routines_05p9(''''%s'''')'');',...
                mod(ii,sap.pm.MaxNumSyllable+1),mod(ii,sap.pm.MaxNumSyllable+1),0,1-(ii+1)*dy,.6,'update_datatmp');
            eval(evaltxt);
        end
        ii=sap.pm.NoiseID; % for noise.
        evaltxt = sprintf('sap.gph.ui_cluster%d = uicontrol(''Style'',''Radio'',''Value'',1,''String'',[''noise'' num2str(%d) '':(--)''],''unit'',''normalized'',''pos'',[%g,%g,%g,dy],''parent'',sap.gph.ui_clusterselect,''HandleVisibility'',''off'',''CallBack'',''ClassifySAP_sub_routines_05p9(''''%s'''')'');',...
            ii,ii,0,1-(ii+1)*dy,.5,'update_datatmp');
        eval(evaltxt);
        ...
        for ii=1:(sap.pm.MaxNumSyllable+2)
            plot(0,-ii,'ko','MarkerFaceColor',sap.pm.CS(ii,:));hold on;
%             pause(0.1)
            if ii==(sap.pm.MaxNumSyllable+2)
                
                text(1,-ii,'=Noise','Color',sap.pm.CS(ii,:));hold on;
            else
        ...
        sap.gph.ui_TgRS= uicontrol('Style','togglebutton',...
            ...
            'CallBack','ClassifySAP_sub_routines_05p9(''open_realsong_mode'');');
        
        
        yh=0.03;
        uicontrol('Style', 'pushbutton','String', 'show syllable',...
            'Unit','Normalized','Position', [.25 .9 .09 yh],'Callback', 'ClassifySAP_sub_routines_05p9(''show_syllable'');');
        
        uicontrol('Style', 'pushbutton','String', 'redraw',...
            'Unit','Normalized','Position', [.35 .9 .09 yh],'Callback', 'ClassifySAP_sub_routines_05p9(''redraw'');');
        
        uicontrol('Style', 'pushbutton','String', 'select&annotate',...
            'Unit','Normalized','Position', [.45 .9 .09 yh],'Callback', 'ClassifySAP_sub_routines_05p9(''select_rect_annotate'');');
        
        
        
        uicontrol('Style', 'pushbutton','String', 'LoadDatabase',...
            'Unit','Normalized','Position', [.55 .9 .09 yh],'Callback', 'ClassifySAP_sub_routines_05p9(''load_database'');');
        
        uicontrol('Style', 'pushbutton','String', 'LoadDB:DAK',...
            'Unit','Normalized','Position', [.55 .96 .09 yh],...
            'Tooltip','Load mysql database generated by .dak data','Callback', 'ClassifySAP_sub_routines_05p9(''load_database_dak'');');
        
        uicontrol('Style', 'pushbutton','String', {'LoadDB:RealSong'},...
            'Unit','Normalized','Position', [.55 .93 .09 yh],...
            'Tooltip','Load mysql database generated by RealSong data','Callback', 'ClassifySAP_sub_routines_05p9(''load_database_realsong'');');
        
        
        uicontrol('Style', 'pushbutton','String', 'UploadDatabase',...
            'Unit','Normalized','Position', [.65 .9 .09 yh],'Callback', 'ClassifySAP_sub_routines_05p9(''upload_database'');');
       
        uicontrol('Style', 'pushbutton','String', 'FastUploadData',...
             'ToolTips','Faster method to upload the whole data into the table.',...
            'Unit','Normalized','Position', [.65 .93 .09 yh],'Callback', 'ClassifySAP_sub_routines_05p9(''fast_upload_database'');');
        
        
        uicontrol('Style', 'pushbutton','String', 'EditAnalysisParam',...
            'Unit','Normalized','Position', [.75 .9 .09 yh],'Callback', 'ClassifySAP_sub_routines_05p9(''open_analysis_parameters'');');
        
        uicontrol('Style', 'pushbutton','String', 'ShowPrincomp',...
            'Unit','Normalized','Position', [.85 .9 .09 yh],'Callback', 'ClassifySAP_sub_routines_05p9(''show_princomp_axis'');');
        ...
        uicontrol('Style', 'pushbutton','String', 'play song',...
            'Unit','Normalized','Position', pos0,'Callback', 'ClassifySAP_sub_routines_05p9(''play_wholesong'');');
        
        uicontrol('Style', 'pushbutton','String', 'print sonogram',...
            'Unit','Normalized','Position', pos1,'Callback', 'ClassifySAP_sub_routines_05p9(''print_sonogram'');');
        ...
        uicontrol('Style', 'pushbutton','String', {'ParseSyllable'},...
            ...
            'Callback','ClassifySAP_sub_routines_05p9(''parse_syllable_transitions'');');
        ...
        uicontrol('Style', 'pushbutton','String', {'TrainSVM'},...
            ...
            'ToolTips','TrainSVM',...
            'Callback','ClassifySAP_sub_routines_05p9(''train_predict_svm'');');
        ...
        uicontrol('Style', 'pushbutton','String', 'ShowTransMat',...
            ...
            'Callback','ClassifySAP_sub_routines_05p9(''show_trans_matrix'');');
        ...
        uicontrol('Style', 'pushbutton','String', {'ShowSyllable','Transition'},...
            ...
            'Callback','ClassifySAP_sub_routines_05p9(''show_syllable_tree'');');
        ...
        uicontrol('Style', 'pushbutton','String', {'AlignSongFeat'},...
            ...
            'Callback','ClassifySAP_sub_routines_05p9(''align_song'');');
        ...
        uicontrol('Style', 'pushbutton','String', {'BoutMotifHist'},...
            ...
            'Callback','ClassifySAP_sub_routines_05p9(''export_bout_durations'');');
        pos1=pos1+[dx,0 0,0];
        uicontrol('Style', 'pushbutton','String', {'Syl&GapHist'},...
            'FontSize',8,...
            'Unit','Normalized','Position', pos1, ...
            'ToolTips','Export syllable gap histgram',...
            'Callback','ClassifySAP_sub_routines_05p9(''export_syllable_gap_hist'');');
        ...
        uicontrol('Style', 'pushbutton','String', {'AlignSonogram'},...
            ...
            'Callback','ClassifySAP_sub_routines_05p9(''align_sonogram_plot'');');
        ...
        uicontrol('Style', 'pushbutton','String', {'DeleteSong'},...
            ...
            'Callback','ClassifySAP_sub_routines_05p9(''delete_song_from_database'');');
        ...
        uicontrol('Style', 'pushbutton','String', {'ExportFeatures'},...
            ...
            'Callback','ClassifySAP_sub_routines_05p9(''export_features_of_a_syllable_type'');');
        pos1=pos1+[0,yh 0,0];
        uicontrol('Style', 'pushbutton','String', {'ExportSyllableFile'},...
            'FontSize',8,...
            'Unit','Normalized','Position', pos1, ...
            'ToolTips','Export parsed syllable data into ASCII format.',...
            'Callback','ClassifySAP_sub_routines_05p9(''export_syllable_file'');');
        ...
        uicontrol('Style', 'pushbutton','String', {'ShowFeatures ToMakeTemplates'},...
            ...
            'Callback','ClassifySAP_sub_routines_05p9(''show_features_to_make_templates'');');
        ...
        uicontrol('Style', 'pushbutton','String', {'ChangeSyllableThreshold'},...
            ...
            'Callback','ClassifySAP_sub_routines_05p9(''change_syllable_threshold'');');
        ...
        uicontrol('Style', 'pushbutton','String', {'UpdateDetectedSyllables'},...
            ...
            'Callback','ClassifySAP_sub_routines_05p9(''update_detected_syllables'');');
        
        
        
        %         ClassifySAP_sub_routines_05p9('init_feature_vector');
        ...
        set(sap.gph.ui_xbuttonG,'SelectionChangeFcn','ClassifySAP_sub_routines_05p9(''chk_xaxis_button'')');
        ...
        % Initialize some button group properties.
        set(sap.gph.ui_ybuttonG,'SelectionChangeFcn','ClassifySAP_sub_routines_05p9(''chk_yaxis_button'')');
        ...
        sap.gph.ui_select_song_by_filename=uicontrol(...
            ...
            'Unit','Normalized',...
            'Position', [.01 .1 .24 .3],...
            'Callback', 'ClassifySAP_sub_routines_05p9(''select_song_by_filename'');');
        ...
        for ii=1:length(sap.pm.feature_names)
            ...
            evaltxt=sprintf('set(%s,''Callback'',''ClassifySAP_sub_routines_05p9(''''chk_feature_button'''')'')',button_uiname);
            eval(evaltxt);
        end
        ...
        sap.gph.ui_shown_cluster_as_noise=uicontrol('Style','pushbutton','String', 'Register as Noise',...
            'FontSize',8,...
            'Unit','Normalized','Position', [.87 .05 .1 .05],'Callback', 'ClassifySAP_sub_routines_05p9(''register_shown_cluster_as_noise'');');
        
        
        sap.gph.ui_register_shown_cluster_to_Manual_cluster=uicontrol('Style','pushbutton','String', 'Register',...
            'Unit','Normalized','Position', [.87 0 .1 .05],'Callback', 'ClassifySAP_sub_routines_05p9(''register_shown_cluster_to_Manual_cluster'');');
        
        sap.gph.ui_spectrum_contrast=uicontrol('Style','slider','Value', 0.1*(1-sap.pm.sonogram_caxis_cutooff),...
            'Unit','Normalized','Position', [.85 0.1 .01 .3],'Callback', 'ClassifySAP_sub_routines_05p9(''set_spectrum_contrast'');');
        ...
        sap.gph.ui_amplitude_thresh=uicontrol('Style','slider','Value', sap.gpm.amplitude_thresh,...
            ...
            'Visible', 'on',...
            'Callback', 'ClassifySAP_sub_routines_05p9(''set_amp_thresh'');');
        
        sap.pm.MotifThreshold=get(sap.gph.ui_amplitude_thresh,'Value')*10;
        sap.pm.MinNoteLength = 10;
    ...
    case 'readonly_database'
        ...
        conn = database(sap.databasename, sap.pm.sql_root, sap.pm.sql_pass);
        setdbprefs('DataReturnFormat','numeric')
        conn_txt=sprintf('select * from %s where cluster<=%d ',sap.SAPtablename_old,sap.pm.MaxNumSyllable+1);
        curs = exec(conn, conn_txt);
        curs = fetch(curs,sap.pm.max_load_N);
        sap.data=curs.data';
        ...
        setdbprefs('DataReturnFormat','cellarray')
        conn_txt=sprintf('select distinct file_name from %s where cluster<=%d ',sap.SAPtablename_old,sap.pm.MaxNumSyllable+1);
        curs = exec(conn, conn_txt);
        curs = fetch(curs,sap.pm.max_load_N);
        ...
        conn_txt=sprintf('select file_name from %s where cluster<=%d  ordered by recnum',sap.SAPtablename_old,sap.pm.MaxNumSyllable+1);
        curs = exec(conn, conn_txt);
        curs = fetch(curs,sap.pm.max_load_N);
    ...
    case {'load_database','load_database_dak','load_database_realsong'}
        ...
        try
            if nargin>=4
                warning('No longer accept sql directory as third variable. Interpret as databasename.');
                sap.databasename=varargin{3};
            else
                datasources=getdatasources;
                 [s,v] = listdlg('PromptString','Select database',...
                    'SelectionMode','single',...
            ...
            if strmatch(ButtonName,'YES')
                ...
                sap.SAPtablename  =EditFromList_GUI(tablenames);
            end
...
%                          'YES', 'NO', 'Cancel', 'YES');
            ...
            %             set(sap.gph.ui_select_song_by_filename,'String', sap.distinct_data_filename,'Value',1);
            conn_txt=sprintf('select file_name from %s where cluster<=%d ',sap.SAPtablename_old,sap.pm.MaxNumSyllable+1);
            ...
            if strmatch(sap.SAPtablename,sap.SAPtablename_old)
                ...
                fprintf('Deleting and making a table %s...\n',sap.SAPtablename);
            ...
            sap.pm.stimulated_index          = strmatch('stimulated',sap.pm.names);
            sap.pm.following_gap_index       = strmatch('following_gap',sap.pm.names);
            sap.pm.file_name_index           = strmatch('file_name',sap.pm.names);
...
%                     sap.pm.sound_dir=sap.pm.feature_dir;
    ...
    case 'upload_database'
        ...
        for kk=1:sap.pm.loaded_N;
            ...
            if kk>(last_kk+sap.pm.loaded_N*0.1)             
        ...
        close(h);
        close(conn);
    case 'fast_upload_database'
        
        conn = database(sap.databasename, sap.pm.sql_root, sap.pm.sql_pass);
              
        varname_in_data = sap.pm.names;
        data_structure=SAP_data_structure_in_MySQL();
        
        % make sure the order of rows are same as in data format.
        sort_index = [];
        
        for ii=1:length(varname_in_data)
            ind = find(strcmp(varname_in_data{ii},{data_structure{:,1}}));
            sort_index = [sort_index,ind];
        end
        
        DataCell = num2cell(sap.data(sort_index,1:sap.pm.loaded_N));
        DataCell(sort_index(sap.pm.file_name_index),:)=sap.data_filename;
        
        [success,is_newtable,sn.database.SAPtablename]=...
            DB_MySQL_control('bulkinsert',conn,...
            sap.SAPtablename,'SAP_data_structure_in_MySQL',...
            DataCell);
    ...
    case 'select_cluster_data_type'
        ...
        ii=sap.pm.MaxNumSyllable+1; % for noise.
    ...
    case 'update_datatmp'
        ...
        for ii=[0:sap.pm.MaxNumSyllable+1]
    ...
    case {'show_syllable','show_syllable_transition','select_song_by_filename','show_syllable_branch'}
        ...
        switch lower(opt)
            ...
            case 'show_syllable_branch'
                ...
                root_pattern='';
                for ss=1:length(search_pattern)
                    if ~strncmp(search_pattern(ss),'+',1)
                        root_pattern=[root_pattern,sprintf('%s+',search_pattern(ss))];
                    end
                end
                ...
                for ss=1:length(search_pattern2)
                    branch_pattern=[branch_pattern,sprintf('%s+',search_pattern2(ss))];
                end
                
                Num_Ns=strfind(str2,'N');
                str2=strrep(str2,'N','');
                %                 search_pattern
                fprintf('look for %s in %s\n',[root_pattern,branch_pattern],str2);
                ending_index=regexp(str2,[root_pattern,branch_pattern],'end');
        ...
        for ii=1:length(sap.pm.same_serial_index)
            ...
            if (is_stim)
                ...
                sap.gph.syllable_line(ii)=line(x-t_offset,[0 0],'Color','g','LineWidth',line_width*0.8,...
                    'ButtonDownFcn','ClassifySAP_sub_routines_05p9(''return_feature_2D_posi'')',...
                ...
                %                         'ButtonDownFcn','ClassifySAP_sub_routines_05p9(''manual_change_cluster'')',...
    ...
    case 'open_analysis_parameters'
        ...
        if FileExists('analysis_parameters.m'); % load converting matrix, etc.
            ...
            copy_txt=sprintf('!copy %s %s',which('ClassifySAP_Default_analysis_parameters_5p6.m'),fullfile(sap.pm.sound_dir,'analysis_parameters.m'));
            eval(copy_txt);
    ...
    case 'reload_converting_matrix'
        current_dir=pwd;
        ClassifySAP_Default_analysis_parameters_5p6;
...
%                 pause;
    ...
    case 'update_cluster_info'
        for ii=[0:sap.pm.MaxNumSyllable]
            evaltxt = sprintf('set(sap.gph.ui_cluster%d, ''String'',[''cluster'' num2str(%d) '':(%d)'']);',...
                mod(ii,sap.pm.MaxNumSyllable+1),mod(ii,sap.pm.MaxNumSyllable+1),length(find(sap.data(sap.pm.cluster_index,:)==mod(ii,sap.pm.MaxNumSyllable+1))));
            eval(evaltxt);
        end
        ii=sap.pm.MaxNumSyllable+1; % for noise.
        evaltxt = sprintf('set(sap.gph.ui_cluster%d,''String'',[''noise'' num2str(%d) '':(%d)'']);',...
    ...
    case 'auto_classify'
        ...
        switch method
            case 'kNN'
                ...
                for ww=1:sap.pm.MaxNumSyllable+1
                ...
                for ii=1:length(sap.pm.same_serial_index)
                    ...
                    if (cluster==0)
                        ...
                        N_nb_clust=hist(nb_clust,[1:sap.pm.MaxNumSyllable+1])./weight;
    ...
    case 'parse_autoclassified_notes'
        ...
        % index starts from 1 and removing Noise (-2), but adding Unknown and END (+2). 
        EndIndex = -2+2+sap.pm.TotalNumCluster; 
        
        sap.pm.trans_matrix=zeros(EndIndex);
        sap.pm.trans_matrix_serials=cell(EndIndex);
        %         val=get(sap.gph.ui_select_song_by_filename,'Value');
        str=get(sap.gph.ui_select_song_by_filename,'String');
        %        sap.pm.filename=str{val};
        ...
        for ii=1:length(str)
            ...
            % use cluser in database or classified 
            which_cluster_type=get(get(sap.gph.ui_cluster_type_select,'selectedobject'),'UserData');
            
            tmp=sap.data(which_cluster_type,sap.pm.same_serial_index);
            tmp(find(tmp==0))=sap.pm.NoiseID; % unclassified one is temporally classified as noise.
 
            % you can change specific sequence into another character.
            % Currently Not used. tmp is changed to character, so needs to
...
%             tmp=tmp2;
            ...
            for jj=1:length(sap.pm.same_serial_index)
                ...
                % if this syllable is too short and not classified as a song, skip it.
                ...
                % and set Inter-syllable-interval as Inf. 
                ...
                if ISI<sap.pm.ISI_thresh % if inter-syllable-interval is more than 200ms (or ISI_thresh), define the end of the song.
                    if (jj==length(sap.pm.same_serial_index))
                        sap.pm.trans_matrix(tmp(jj),EndIndex)=ceil(sap.pm.trans_matrix(tmp(jj),EndIndex)+1);
                        sap.pm.trans_matrix_serials{tmp(jj),EndIndex}=[sap.pm.trans_matrix_serials{tmp(jj),EndIndex},...
            ...
            if(is_within_bout)
                sap.pm.trans_matrix(tmp(end),EndIndex)=ceil(sap.pm.trans_matrix(tmp(end),EndIndex)+1);
                sap.pm.trans_matrix_serials{tmp(end),EndIndex}=[sap.pm.trans_matrix_serials{tmp(end),EndIndex}, sap.pm.same_serial_index(end)];
            end
        end
        close(h)
        
        show_index=[];
        % eliminate cluster which does not have teacher data.
        for ii=1:(sap.pm.MaxNumSyllable+1)
        ...
        % add N
%         show_index_x=unique([show_index,sap.pm.MaxNumSyllable,length(sap.pm.converting_matrix)])
        show_index      =setdiff(show_index,sap.pm.NoiseID); % remove noise part.
        show_index_y    =show_index;
        show_index_x    =unique([show_index, sap.pm.MaxNumSyllable+1, size(sap.pm.trans_matrix,2)]);
        sap.pm.show_index_y_for_sylltrans=show_index_y;
        sap.pm.show_index_x_for_sylltrans=show_index_x;
        
        tmp=sap.pm.converting_matrix;
        labels=[tmp(2:end)];
        sap.pm.Ntrans.labels_X=[labels(show_index_x(1:end-2)),'N','End'];
        sap.pm.Ntrans.labels_Y=labels(show_index_y);
    ...
    case 'parse_autoclassified_notes_strict'
        %     Needs to be updated if it is still used.
        ...
        for ii=1:length(str)
            ...
            tmp=sap.data(which_cluster_type,sap.pm.same_serial_index);
            tmp(find(tmp==1))=sap.pm.MaxNumSyllable+1; % introductory note is temporally classified as noise.
            tmp(find(tmp==0))=sap.pm.MaxNumSyllable+1; % unclassified one is temporally classified as noise.
            tmp=filter_syllable_structure(sprintf('%d',tmp),sap.pm.replace_chart_in_index,{});
        ...
        show_index_x=unique([show_index,sap.pm.MaxNumSyllable+1,10]);
        show_index=setdiff(show_index,sap.pm.MaxNumSyllable+1); % remove noise part.
        show_index_y=show_index;
        sap.pm.show_index_y_for_sylltrans=show_index_y;
        sap.pm.show_index_x_for_sylltrans=show_index_x;
...
%         labels_X=labels(show_index_x);
%         labels_Y=labels(show_index_y);
        show_index = sap.pm.show_index_y_for_sylltrans;
        labels_X = sap.pm.Ntrans.labels_X;
        labels_Y = sap.pm.Ntrans.labels_Y;
        ...
        uicontrol('Style', 'pushbutton','String', 'show syllable','Parent',sap.gph.ui_Transmat_panel,...
            ...
            'Callback', 'ClassifySAP_sub_routines_05p9(''choose_syllable_transition'');');
        sap.ui.decrement_transit_syllable=uicontrol('Style', 'pushbutton','String', '<','Parent',sap.gph.ui_Transmat_panel,...
            ...
            'Callback', 'ClassifySAP_sub_routines_05p9(''move_syllable_transition'',-1);');
        sap.ui.edit_transit_syllable=uicontrol('Style', 'edit','String', '1','Parent',sap.gph.ui_Transmat_panel,...
            ...
            'Callback', 'global sap; n=str2num(get(sap.ui.edit_transit_syllable,''String''));ClassifySAP_sub_routines_05p9(''move_syllable_transition'',n);');
...
%         figure(20);
    ...
    case 'manual_change_cluster'
        ...
        sap.gph.uieditnow=uicontrol('Style', 'edit','String', get(gcbo,'String'),...
            ...
            'UserData',tmp,...
            'Callback', 'ClassifySAP_sub_routines_05p9(''enter_cluster_number'');');
    ...
    case  'register_shown_cluster_to_manual_cluster'
        ...
        ButtonName=questdlg('Do you want to register the clusters shown below as the manually clustered value?', ...
            'Genie Question', ...
            'YES(All)','YES(only within the view)','Cancel','YES(All)');
        
         
        switch ButtonName
            case {'YES(All)','YES(only within the view)'}
                do_register = 1;   
                switch ButtonName
                    case 'YES(All)'
                        index =1:length(sap.gph.syllable_label);
                    case 'YES(only within the view)'
                        xrange=get(sap.gph.sonogramAxes,'XLim');     
                        start_on=sap.data(sap.pm.start_on_index,sap.pm.same_serial_index)/1000;    
                        index =find(start_on>=min(xlim) & start_on <= max(xlim));
                    otherwise
                        error('Unknown button %s',ButtonName)
                end
            case 'Cancel'
                disp('User Canceled');
                do_register = 0;
...
%         index
        fprintf('Registering:')
        if (do_register) & ~isempty(index)
            for ii=index
                if ishandle(sap.gph.syllable_label(ii))
    ...
    case  'register_shown_cluster_as_noise'
        ...
        if strmatch(ButtonName,'YES')
            ...
            for ii=1:length(sap.gph.syllable_label)
                set(sap.gph.syllable_label(ii),'String',num2str(sap.pm.MaxNumSyllable+1));
                index=get(sap.gph.syllable_label(ii),'UserData');
                sap.data(sap.pm.cluster_index,index(1))=sap.pm.MaxNumSyllable+1; % register as a noise
                sap.data(sap.pm.classified_cluster_index,index(1))=sap.pm.MaxNumSyllable+1; % register as a noise
    ...
    case 'parse_syllable_transitions'
        ...
        sap.data(which_cluster_type,nan_index)=sap.pm.MaxNumSyllable+1;
...
%       % same_serial_index must be also sorted in ascending order of start_on.
    ...
    case 'show_syllable_tree'
        ...
        prompt={'Enter the first (root) syllables that you want to restrict search. Note that repeats (trills) are neglected in this search.'};
    ...
    case 'select_rect_annotate'
        ...
        if (cluster<0)||(cluster>sap.pm.MaxNumSyllable+1) || ~isscalar(cluster)
            error(sprintf('cluster number should be [0-%d]',sap.pm.MaxNumSyllable+1));
        end
    ...
    case 'estimate_gaussian_mixture'
        ...
        W=zeros(1,sap.pm.MaxNumSyllable+1);
        for ii=1:sap.pm.MaxNumSyllable+1
    ...
    case 'kmeans_clustering'
        ClassifySAP_sub_routines_05p9('estimate_gaussian_mixture');
    ...
    case 'em_clustering'
        ...
        %         sap.pm.MaxNumSyllable+1 is the noise
        %
    ...
    case 'train_predict_svm'
        ...
        sap.pm.model=struct;
        fprintf('training...\n')
        % find unique class labels including sub-classes such as 2.1, 2.2,
        % they are all recognizeda as class 2, but labeled as 2.1. 
        
        sap.pm.unique_classes=unique(sap.data(sap.pm.cluster_index,:));
        sap.pm.unique_classes=sap.pm.unique_classes(sap.pm.unique_classes>0);
        
        for class=1:sap.pm.MaxNumSyllable+1
        ...
        sap.pm.accuracy=zeros(3,sap.pm.MaxNumSyllable+1);
        for class=1:sap.pm.MaxNumSyllable+1
...
%                     'pgood'             ,[0 500]};
            ...
            %     plot variables |  yrange
                varnames={...
                    ...
                    'pitch'             ,[0 2000],      'pitch';...
                    'pgood'             ,[0 500],       'pgood';....
                    'precisepitch'      ,[0 2000],      'ppitch';... 
                    'MainSylGate'       ,[-0.1 1.1],    'sylgate';...
                    'Predictor'         , [-0.1 1.1],   'predict'};
        ...
        for val=1:length(str)
            ...
            [is_exist_this_pattern,...
                pattern_start_ind,...
                Endline_ind,...
                pattern_end_ind,...
                detected_song_string]=...
                    ...
                    if (do_calc_features_offline)
                        ...
                        switch sap.pm.data_type
                            ...
                            case 'realsong'
                                ...
                                sap.pm.feature.precisepitch    =rsd.precisepitch;
                                sap.pm.feature.Pitch_chose =[];
                                sap.pm.feature.Pitch_weight=[];
                                sap.pm.feature.f           = [];
...
%  Update: 20140904. 
%  Now, syl_onsets_all and syl_offsets_all do not include stimulus onset.                                       
                    detected_song_string_tmp = detected_song_string{ll};
                    
                      
                    sap.pm.show_sonogram_index=same_serial_index(correct_start_ind+align_number-1);
                    NotStim = sap.data(sap.pm.cluster_index,same_serial_index)>=0;
                    same_serial_index_wostim = same_serial_index(NotStim);
                    
                    pattern_index=correct_start_ind:(correct_start_ind+align_number-1);
                    syl_onsets_all  = sap.data(sap.pm.start_on_index,same_serial_index_wostim)/1000;
                    syl_onsets  = syl_onsets_all(pattern_index);
                                        
                    syl_offsets_all  = sap.data(sap.pm.start_on_index,same_serial_index_wostim)/1000+...
                        sap.data(sap.pm.duration_index,same_serial_index_wostim)/1000;
...
%                          
                            ...
                            variables_to_store={...
                                ...
                                'filename',       'sap.pm.filename',    'string';...
                                'detected_song_string','detected_song_string_tmp','string';...
                                'syl_onsets_all', 'syl_onsets_all',   'row_vector';...
                                'syl_onsets',     'syl_onsets',       'row_vector';...
                                'syl_offsets_all','syl_offsets_all',  'row_vector';...
                                'syl_offsets',    'syl_offsets',      'row_vector';...
...
% %                                         tind_feature_s_e_diff
        ...
        switch data_type,
            case {'wav','cbr','realsong'}
                ...
                print_detect_pattern=strrep(print_detect_pattern,'*','STAR');
                
                savename=fullfile(sap.pm.analysis_dir,...
                    sprintf('%s_%s_%s_%d_%s_N=%d',...
                    deblank(save_sub_var{kk}),save_ffr,print_detect_pattern,align_number,...
                    align_pos,jj-1));
        ...
        str=sprintf('%s_',save_sub_var{:}); str=str(1:end-1);
        ...
        t_vec=sap.pm.t_vec;
        fprintf('saving data in %s \n(also filename on Clipboard)\n',savename);
        save([savename,'.mat'],'aligndata','t_vec','-v7.3');
        savename_split=strsplit(savename,filesep);
        clipboard('copy', fullfile(savename_split{end-1:end})) ;
...
%             same_serial_index = strmatch(sap.pm.filename,sap.data_filename);
        ...
        uicontrol('Style', 'pushbutton',...
            ...
            'Callback', 'ClassifySAP_sub_routines_05p9(''export_aligned_feature_within_xaxis'');');
        ...
        if isempty(songstr)
            ...
            if strncmp(detect_pattern,'\<',2)
                fprintf('look for %s at the beginning of %s\n',detect_pattern,songstr);
                 pattern_start_ind=regexp(songstr,detect_pattern,'start');
                 pattern_end_ind=regexp(songstr,detect_pattern,'end');
            else % normal case 
                fprintf('look for %s in %s\n',detect_pattern,songstr);
                 pattern_start_ind=regexp(songstr,detect_pattern,'start');
                 pattern_end_ind=regexp(songstr,detect_pattern,'end');
            ...
            detected_strings = {};
            for ii=1:length(pattern_start_ind)
                detected_strings{ii}=songstr(pattern_start_ind(ii):pattern_end_ind(ii));
            end
            ...
            varargout{4}=pattern_end_ind;
            varargout{5}=detected_strings;
        end
...
% If the format is like 'August_13_2013_RANDOM_SERIAL'
if isempty(rec_date)
    fprintf('Unknown format filename: Gave up to sort files in temporal order.')
    sorted_recdate = [];
    sorted_recdate_ind=1:length(distinct_data_filename);
end
