function [mm_spec_deriv ,...
          ...
          mm_Pitch_chose ,...
          mm_Pitch_weight,...
          f ,t,param]=sam_spectrum_deriv8(TS,fs,varargin)


% [m_spec_deriv , m_AM, m_FM ,m_Entropy , m_amplitude ,m_frequency, m_PitchGoodness , m_Pitch , Pitch_chose , Pitch_weight ]=sam_spectrum_deriv3(TS,fs);
...
cepst_nfft=512;         % nfft size for cepstrum;
quefrency = [0:cepst_nfft]*(param.pad)/(fs*cepst_nfft); % quefrency. in [sec].
quefrency_select=(1./quefrency<8000 & 1./quefrency > 400); % select quefrency in 400 to 8000 Hz range.
...
hanning_matrix=repmat(hanning(length(select))',size(m_spec_deriv,1),1);

Cepstrum=(fft(hanning_matrix.*m_spec_deriv./(m_powSpec(:,select)+eps),cepst_nfft,2))*( 1/2);
...
for kk=1:length(ismember_index)
    ...
    plot(1/quefrency(m_Pitch(test_index))*[1 1], [ylim]);
...
% 
% Pitch_chose= fs./(m_Pitch-1)/2;
Pitch_chose= 1./qt(m_Pitch);

% figure(2);clf;
% subplot(3,1,1);h=plotyy(1:length(Pitch_chose),Pitch_chose,1:length(Pitch_chose),m_Entropy);ylabel('Pitch & Entropy');hold on;
% set(h(2),'YLim',[-20 0],'YTick',[-20 -10 -5 0]);% subplot(3,1,2);plot(Pitch_chose,'k-');ylabel('Pitch chose');hold on;
% subplot(3,1,3);imagesc(x');
...
% subplot(3,1,2);plot(m_frequency,'r-');hold on;axis tight;
% plot(m_PitchGoodness,'g-');hold on;
