function [out,varargout]=TemplateMaker_subroutine_0p2(opt,varargin)
...
%         s{5}='meanfreq';
    ...
    case 'init_fig'
        ...
        tmkr.gph.amp_slider=uicontrol('Style', 'slider',...
            ...
            'Min',  0,...
            'Position',pos,...
            'Callback', 'TemplateMaker_subroutine_0p2(''set_amp_slider'');');
        ...
        tmkr.gph.amp_edit=uicontrol('Style', 'edit',...
            ...
            'Callback', 'TemplateMaker_subroutine_0p2(''set_amp_edit'');');
        ...
        uicontrol('Style', 'pushbutton','Parent',tmkr.gph.template_ui_panel,'String', 'play',...
            'Unit','Normalized','Position', pos,'Callback', 'TemplateMaker_subroutine_0p2(''play'');');
   
        pos=origin+[dx -2*dy 1-2*dx, dh];
        uicontrol('Style', 'text','Parent',tmkr.gph.template_ui_panel,...
            'String', 'spectrum calc method',...
           'Unit','Normalized','Position', pos);
        
        
        pos=origin+[dx -3*dy 1-2*dx, dh];
        tmkr.gph.ui_spectmethod= uicontrol('Style', 'popupmenu','Parent',tmkr.gph.template_ui_panel,'String', {'sFFT','multitaper_features','sam_spectrum_deriv5'},...
            'Value',1,'Unit','Normalized','Position', pos,'Callback', 'TemplateMaker_subroutine_0p2(''select_spect_calc_method'');');
        feval(mfilename,'select_spect_calc_method');
       
        pos=origin+[dx -4*dy 1-2*dx, dh]; 
        uicontrol('Style', 'pushbutton','Parent',tmkr.gph.template_ui_panel,'String', 'train template',...
            'Unit','Normalized','Position', pos,'Callback', 'TemplateMaker_subroutine_0p2(''train_template'');');
    
        pos=origin+[dx -5*dy 1-2*dx, dh]; 
        uicontrol('Style', 'pushbutton','Parent',tmkr.gph.template_ui_panel,'String', 'predict template',...
            'Unit','Normalized','Position', pos,'Callback', 'TemplateMaker_subroutine_0p2(''predict_template'');');
    
         pos=origin+[dx -6*dy 1-2*dx, dh]; 
        uicontrol('Style', 'pushbutton','Parent',tmkr.gph.template_ui_panel,'String', 'reset template',...
            'Unit','Normalized','Position', pos,'Callback', 'TemplateMaker_subroutine_0p2(''reset_template'');');
        
        pos=origin+[dx -7*dy 1-2*dx, dh]; 
        uicontrol('Style', 'pushbutton','Parent',tmkr.gph.template_ui_panel,'String', 'save template',...
            'Unit','Normalized','Position', pos,'Callback', 'TemplateMaker_subroutine_0p2(''save_template'');');
    
            pos=origin+[dx -8*dy 1-2*dx, dh]; 
        uicontrol('Style', 'pushbutton','Parent',tmkr.gph.template_ui_panel,'String', 'load template',...
            'Unit','Normalized','Position', pos,'Callback', 'TemplateMaker_subroutine_0p2(''load_template'');');
        
               pos=origin+[dx -9*dy 1-2*dx, dh]; 
        uicontrol('Style', 'pushbutton','Parent',tmkr.gph.template_ui_panel,'String', 'load song',...
            'Unit','Normalized','Position', pos,'Callback', 'TemplateMaker_subroutine_0p2(''load_song'');');
...
%             tmkr.pm.feature2.f=tmkr.pm.Fsong*(1:size(tmkr.pm.feature.m_spec_deriv ,2))/1024;
    ...
    case 'train_template'
        ...
        Y=2*train_label-1;
        X=[tmkr.template.target_data,tmkr.template.nontarget_data];
        sigma2=1/2;
        tmkr.template.sv=my_trainsvm(X,Y,sigma2);
    ...
    case 'predict_template'
        ...
        [g_predict,b] = my_predictsvm(tmkr.template.sv, x);
...
%         plot(tmkr.pm.feature2.t,predicted_index*max(ylim),'g');
    ...
    case 'reset_template'
        ...
        if isfield(tmkr,'data') % There are distribution data.
            tmkr.template.template_scaling_factor=std(tmkr.data(tmkr.pm.template_classify_dim_index,:),[],2);
        else
            template_classify_features={...
                'Duration',...
                'Entropy',...
                'FM',...
                'Pitch',...
            ...
            tmkr.template.template_scaling_factor=[...
                10,... % duration
                3,...  % Entropy
                1,...  % FM
                1,...  % Pitch
...
%                 tmkr.template.SamplingFreq=
    ...
    case 'draw_sonogram_features'
        ...
        switch tmkr.pm.specmethod
            case 'sFFT'
                
                
                nWinsize=512;
                nFFT=1024;
                nOverlap=nWinsize-60;
                [s,f,t,p]=spectrogram(tmkr.pm.song,nWinsize,nOverlap,nFFT,tmkr.pm.Fsong);
                tmkr.pm.feature2.t=t;
                tmkr.pm.feature2.f=f;
                tmkr.pm.feature2.m_spec_deriv=p';
                [tmkr.pm.feature2.amplitude,...
                    tmkr.pm.feature2.meanfreq,...
                    tmkr.pm.feature2.entropy,...
                    tmkr.pm.feature2.pitch] =...
                    calc_feature_from_powerspec(p,nWinsize,nFFT,tmkr.pm.Fsong);
                amp_bias=70;
...
%                     tmkr.pm.feature2.meanfreq, ...
%                     tmkr.pm.feature2.pgood , ...
%                     tmkr.pm.feature2.pitch]=calc_feature_from_powerspec;
            case 'multitaper_features'
                
                
                nWinsize=512;
                nFFT=1024;
                nstepsize=60;
                nOverlap=nWinsize-nstepsize;
                
                nsteps=floor((length(tmkr.pm.song)-nWinsize)/nstepsize)+1;
                
                tmkr.pm.feature2.t=[nWinsize/2:nOverlap:(nWinsize+(nsteps-1)*nOverlap)]/tmkr.pm.Fsong;
                f=tmkr.pm.Fsong*((1:nFFT)-1)/nFFT;
                frange=[600,8000];
                findex=find(f>=frange(1)&f<=frange(2));
                tmkr.pm.feature2.f=f(findex);
                
                
                tmkr.pm.feature2.m_spec_deriv=zeros(length(findex),nsteps);
                tmkr.pm.feature2.amplitude  = zeros(1,nsteps);
                tmkr.pm.feature2.entropy    = zeros(1,nsteps);
                tmkr.pm.feature2.FM         = zeros(1,nsteps);
                tmkr.pm.feature2.AM         = zeros(1,nsteps);
                tmkr.pm.feature2.meanfreq   = zeros(1,nsteps);
                tmkr.pm.feature2.pitch      = zeros(1,nsteps);
                tmkr.pm.feature2.pitchgoodness=zeros(1,nsteps);
                ...
                for ii=1:nsteps
                    ...
                    J1=fft(tmkr.pm.song(index).*Ec,nFFT);
                    J2=fft(tmkr.pm.song(index).*Es,nFFT);
     
                    
                    [  tmkr.pm.feature2.m_spec_deriv(:,ii),...
                       tmkr.pm.feature2.amplitude(ii),...
                       tmkr.pm.feature2.entropy(ii),...
                       tmkr.pm.feature2.FM(ii),...
                       tmkr.pm.feature2.AM(ii),...
                       tmkr.pm.feature2.meanfreq(ii),...
                       tmkr.pm.feature2.pitch(ii),...
                       tmkr.pm.feature2.pitchgoodness(ii)] = ...
                        multitaper_feature_RTSAC(J1(:),J2(:),1/tmkr.pm.Fsong,nFFT,frange);
            ...
            otherwise
                error('Unknown spectrum calculation method');
        end
        ...
        colormap bone;
        axis([0,tmkr.pm.feature2.t(end),0,8000]);
                ...
                if counter(ii)==0;
                    ...
                    for jj=1:length(tmkr.template.feature_names)
                        ...
                        x(jj)=features.(tmkr.template.feature_names{jj})(ii);
                            ...
                            error('%s error',tmkr.template.feature_names{jj});
                        end
                    end
...
%         set(tmkr.gph.template_feature.mtf_region,'XData',px,'YData',py);
        ...
        for ii=1:length(tmkr.pm.feature2.start_ind)
            ...
            tmkr.gph.motifpatch_for_template(ii)=patch(px(:,ii),py(:,ii),'b',...
                'FaceAlpha',0.5,...
                'ButtonDownFcn','TemplateMaker_subroutine_0p2(''Toggle_Target_or_Not'')',...
    ...
    case 'select_spect_calc_method'
        value=get(tmkr.gph.ui_spectmethod,'Value');
        str  = get(tmkr.gph.ui_spectmethod,'String');
        fprintf('%s selected as spectrum calculation method.\n',str{value});
        tmkr.pm.specmethod=str{value};
