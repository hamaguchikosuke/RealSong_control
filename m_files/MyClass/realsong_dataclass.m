classdef realsong_dataclass < handle
% RealSong Data class.
...
% by KH 20120808
    ...
    methods
        function obj = realsong_dataclass(filename,varargin)
            ...
            if isempty(speaker_index)
                ...
                obj.speaker=cbdata(:,speaker_index+1);
            end
            
            % If there are Feature data 
            if isfield(pm,'feat')
                init_index=find(cbdata(:,pm.n_rec_channel+1)==pm.initvec_val);
                for ii=1:size(pm.features_vec_names,1);
                    name=pm.features_vec_names{ii,1};
                    indp=init_index+pm.feat.([name '_index']);
                    if indp(end)>size(cbdata,1)
                        indp=indp(1:end-1);
                        obj.(name)=cbdata(indp,pm.n_rec_channel+1);
                ...
                obj.meanfreq = obj.frequency; % some program needs it as frequency, but same thing.
            end
