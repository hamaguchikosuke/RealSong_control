function varargout = songedit_03p2(varargin)
...
% Last Modified by GUIDE v2.5 25-Nov-2013 16:48:26
...
if strncmp(filename(end-3:end),'.wav',4)    
    ...
    fprintf('Loading %s (%5.0fHz,%2.0fbit)\n',filename,handles.os.F,handles.os.NBits);
elseif strncmp(filename(end-3:end),'.mat',4) 
    load(fullfile(pathname,filename));
     handles.os.song=[stim(:).song];
     handles.os.F = stim(1).Fsong;
     handles.os.NBits = 16;
    fprintf('Loading MAT-file %s (%5.0fHz,%2.0fbit)\n',filename,handles.os.F,handles.os.NBits);
end
...
switch lower(songtype)
    case 'edit'
        songtype=get(handles.edit_stim_name,'String');
    otherwise
end

do_resample=get(handles.rb_resample,'Value');
target_samprate=round(str2double(get(handles.edit_target_samprate,'String')));

if (do_resample)
    if round(handles.os.F)~=round(target_samprate)
        fprintf('Resampling sound to %5.5fHz and save data.',target_samprate);
        songtmp=resample(handles.ms.song,target_samprate,handles.os.F);
        Fsongtmp=target_samprate;
        ttmp=linspace(handles.ms.t(1),handles.ms.t(end),length(songtmp));
    else
        songtmp=handles.ms.song;
        Fsongtmp=handles.os.F;
        ttmp=handles.ms.t;
    end
else
    songtmp=handles.ms.song;
...
switch val
    case 1 % new file
        [handles.savename,handles.savedir]=uiputfile;
        stim(1).song        =songtmp;
        stim(1).t           =ttmp;
        stim(1).type        =songtype;
        stim(1).Fsong       =Fsongtmp;
     
    case 2
       try
            [handles.savename,handles.savedir]=uiputfile(fullfile(handles.savedir,handles.savename),'Append file');
       catch
            [handles.savename,handles.savedir]=uigetfile({'*.mat';'*.*'},'Pick an append file');
        end
        load(fullfile(handles.savedir,handles.savename));
        n=length(stim);
        stim(n+1).song      =songtmp;
        stim(n+1).t         =ttmp;
        stim(n+1).type      =songtype;
        stim(n+1).Fsong     =Fsongtmp;
       
    otherwise
        error('Unknown option!')
end
 save(fullfile(handles.savedir,handles.savename),'stim');
guidata(hObject,handles);
...
switch lower(contents{get(hObject,'Value')})
    case 'edit'
        set(handles.edit_stim_name,'Enable','On');
    otherwise
        set(handles.edit_stim_name,'Enable','Off');
end
...
function edit_target_samprate_Callback(hObject, eventdata, handles)
% hObject    handle to edit_target_samprate (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of edit_target_samprate as text
%        str2double(get(hObject,'String')) returns contents of edit_target_samprate as a double


% --- Executes during object creation, after setting all properties.
function edit_target_samprate_CreateFcn(hObject, eventdata, handles)
% hObject    handle to edit_target_samprate (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in rb_resample.
function rb_resample_Callback(hObject, eventdata, handles)
% hObject    handle to rb_resample (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of rb_resample


% --- Executes on button press in pb_loadsong_now.
function pb_loadsong_now_Callback(hObject, eventdata, handles)
% hObject    handle to pb_loadsong_now (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles = pb_loadsong_Callback(hObject, eventdata, handles);
guidata(hObject,handles);
...
% --- Executes on button press in pb_overlayNoise.
function pb_overlayNoise_Callback(hObject, eventdata, handles)
% hObject    handle to pb_overlayNoise (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
Trange = input('overlay range [sec] (1x2) vector:');
Trange_index = ceil(Trange*handles.ms.F)+1;
Trange_index  = Trange_index(1):Trange_index(2);

if Trange(2)>handles.ms.t(end)
    error('Time range cannot exceed current song duration (%f)',handles.ms.t(end));
end
...
handles.ms.song(Trange_index)=...
handles.ms.song(Trange_index)+sqrt(nvar)*randn(1,length(Trange_index))+nmean;
